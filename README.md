...
Hints climb a ladder, one rung per failed fix, the way a human tutor escalates — and they start as small as a hint can be. The first rung is a flag: the kind of mistake and roughly where, a few words, because an error you hunt down yourself teaches more than one explained to you. If your fix fails, the next rung names the constraint the step violates — the general law it breaks, stated so it holds with any numbers, never applied to yours; even an arithmetic slip carries its governing law. The rung after states what that constraint requires along your own route — a case distinction, an isolated variable — never a resulting value. The last points at the printed solutions for that one step; explaining it to yourself and rewriting it in your own words is what makes it stick. Every sentence is a statement, never an order, and kept as short as the rung allows — the tutor describes, and leaves the doing to you. A question mark written next to a flagged spot advances the ladder without waiting for a failed attempt.
